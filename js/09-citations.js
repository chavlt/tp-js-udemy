...
let authorName = document.querySelector('#auteur');
let buttonNewCitation = document.querySelector('#nouveau');

let dernier = 0;
let nombreDeCitations = citations.length;
let nombreAleatoire;



buttonNewCitation.addEventListener('click', () => {
  do{
    nombreAleatoire = genererNombreEntier(nombreDeCitations);
  }while(nombreAleatoire == dernier);

  console.log(citations[nombreAleatoire][0]);
  console.log(nombreAleatoire);
  citationText.innerHTML = citations[nombreAleatoire][0];
  authorName.innerHTML = citations[nombreAleatoire][1];
})


/* FUNCTIONS */

// Générer un nombre aléatoire entier compris entre 0 et le nombre maximal de citation dans mon tableau
